...
## 概述
添加包含免费/专业�?企业版三个等级的订阅计费功能。用户通过 Stripe Checkout 进行升级，Webhook 事件将保持订阅状态的同步�?

## 需�?
- 三个等级：免费（默认）、专业版�?9美元/月）、企业版�?9美元/月）
- 使用 Stripe Checkout 完成支付流程
- 用于处理订阅生命周期事件�?Webhook 处理�?
- 基于订阅等级的功能权限控�?

## 架构变更
- 新表：`subscriptions` (user_id, stripe_customer_id, stripe_subscription_id, status, tier)
- �?API 路由：`app/api/checkout/route.ts` �?创建 Stripe Checkout 会话
- �?API 路由：`app/api/webhooks/stripe/route.ts` �?处理 Stripe 事件
- 新中间件：检查订阅等级以控制受保护功�?
- 新组件：`PricingTable` �?显示等级信息及升级按�?

## 实施步骤

### 阶段 1：数据库与后�?(2 个文�?
1. **创建订阅数据迁移** (文件：supabase/migrations/004_subscriptions.sql)
    - 操作：使�?RLS 策略 CREATE TABLE subscriptions
    - 原因：在服务器端存储计费状态，绝不信任客户�?
    - 依赖：无
    - 风险：低

2. **创建 Stripe webhook 处理�?* (文件：src/app/api/webhooks/stripe/route.ts)
    - 操作：处�?checkout.session.completed、customer.subscription.updated、customer.subscription.deleted 事件
    - 原因：保持订阅状态与 Stripe 同步
    - 依赖：步�?1（需�?subscriptions 表）
    - 风险：高 �?webhook 签名验证至关重要

### 阶段 2：Checkout 流程 (2 个文�?
3. **创建 checkout API 路由** (文件：src/app/api/checkout/route.ts)
    - 操作：使�?price_id �?success/cancel URL 创建 Stripe Checkout 会话
    - 原因：服务器端会话创建可防止价格篡改
    - 依赖：步�?1
    - 风险：中 �?必须验证用户已认�?

4. **构建定价页面** (文件：src/components/PricingTable.tsx)
    - 操作：显示三个等级，包含功能对比和升级按�?
    - 原因：面向用户的升级流程
    - 依赖：步�?3
    - 风险：低
...
* **阶段 1**：最小可行产�?�?能提供价值的最小切�?
* **阶段 2**：核心体�?�?完成主流程（Happy Path�?
* **阶段 3**：边界情�?�?错误处理、边界情况、细节完�?
* **阶段 4**：优�?�?性能、监控、分�?
...
**请记�?*：一个好的计划是具体的、可操作的，并且同时考虑了正常路径和边缘情况。最好的计划能确保自信、增量的实施�?
