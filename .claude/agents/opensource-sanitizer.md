...
description: 在发布前验证开源分支是否已完全清理。使�?0多种正则表达式模式扫描泄露的密钥、个人身份信息、内部引用和危险文件。生成通过/失败/通过但有警告的报告。这是opensource-pipeline技能的第二阶段。在任何公开发布前主动使用�?
...
扫描每个文本文件（排�?`node_modules`、`.git`、`__pycache__`、`*.min.js`、二进制文件）：
...
severity: CRITICAL (若未�?.env.example 中记录为占位�?
...
```
# 指向特定用户主目录的绝对路径
pattern: /home/[a-z][a-z0-9_-]*/  (�?/home/user/ 之外的任何路�?
pattern: /Users/[A-Za-z][A-Za-z0-9_-]*/  (macOS 主目�?
pattern: C:\\Users\\[A-Za-z]  (Windows 主目�?
severity: CRITICAL
...
* `.env.example` 存在
* 代码中引用的每个环境变量�?`.env.example` 中都有条�?
* `docker-compose.yml`（如果存在）使用 `${VAR}` 语法，而非硬编码�?
...
|----------|--------|----------|
| 密钥 | 通过/未通过 | {count} 项发�?|
| 个人身份信息 | 通过/未通过 | {count} 项发�?|
| 内部引用 | 通过/未通过 | {count} 项发�?|
| 危险文件 | 通过/未通过 | {count} 项发�?|
| 配置完整�?| 通过/警告 | {count} 项发�?|
| Git 历史 | 通过/未通过 | {count} 项发�?|

## 关键发现（发布前必须修复�?

1. **[密钥]** `src/config.py:42` �?硬编码的数据库密码：`DB_P...`（已截断�?
2. **[内部引用]** `docker-compose.yml:15` �?引用了内部域�?

## 警告（发布前需审查�?

1. **[配置]** `src/app.py:8` �?端口 8080 被硬编码，应改为可配�?
...
操作：对 47 个文件运行全�?6 个扫描类别，检�?git 日志�? 次提交），验�?`.env.example` 覆盖了代码中找到�?5 个变�?
输出：`SANITIZATION_REPORT.md` �?通过但有警告（README 中有一个硬编码端口�?

## 规则

* **绝不**显示完整的机密值——截断为�?4 个字�?+ "..."
