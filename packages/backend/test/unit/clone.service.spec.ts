...
describe('CloneService', () => {
    ...
    describe('buildCloneUrl', () => {
        it('无代理配置 → 附加 .git 后缀', () => {
            ...
            expect(service.buildCloneUrl('https://github.com/owner/repo')).toBe('https://github.com/owner/repo.git');
        });

        it('代理 URL 无尾部斜杠 → 中间补 "/"', () => {
            mockConfigService.getValueDefault.mockReturnValue('https://mirror.example.com');
            expect(service.buildCloneUrl('https://github.com/owner/repo')).toBe('https://mirror.example.com/https://github.com/owner/repo');
        });
    ...
    describe('getTask', () => {
        ...
        it('任务存在 → 通过 findUnique 查询并返回', async () => {
            ...
            expect(mockPrisma.cloneTask.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { taskId: 'c1' } }));
        });
    });
