...
describe('StatsService', () => {
    ...
    describe('getLanguageStats', () => {
        ...
        it('单一语言 → 百分比为 100', async () => {
            mockPrisma.githubRepo.groupBy.mockResolvedValue([{ language: 'Java', _count: { id: 42 } }]);
        ...
        it('total 为 0 时百分比为 0（避免除零）', async () => {
            ...
            mockPrisma.githubRepo.groupBy.mockResolvedValue([{ language: 'Go', _count: { id: 5 } }]);
            mockPrisma.githubRepo.count.mockResolvedValue(0);
    ...
    describe('getOwnerStats', () => {
        ...
        it('_max.ownerAvatarUrl 为 null 时返回空字符串', async () => {
            mockPrisma.githubRepo.groupBy.mockResolvedValue([{ ownerName: 'noavatar', _count: { id: 1 }, _max: { ownerAvatarUrl: null } }]);
    ...
    describe('getTimelineStats', () => {
        ...
        it('bigint count 正确转换为 Number', async () => {
            mockPrisma.$queryRaw.mockResolvedValue([{ month: '2025-06', count: BigInt(9999999) }]);
    ...
    describe('getOverviewStats', () => {
        it('返回正确的聚合数据', async () => {
            ...
            mockPrisma.githubRepo.findMany
                ...
                .mockResolvedValueOnce([{ ownerName: 'alice' }, { ownerName: 'bob' }, { ownerName: 'charlie' }]); // 第2次调用：distinct owners
        ...
        it('stars/forks 聚合结果为 null 时返回 0', async () => {
            ...
            mockPrisma.githubRepo.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
        ...
        it('bigint → Number 正确转换', async () => {
            ...
            mockPrisma.githubRepo.findMany.mockResolvedValueOnce([{ language: 'TS' }]).mockResolvedValueOnce([{ ownerName: 'x' }]);
        ...
        it('5 个查询应并行执行（Promise.all）', async () => {
            ...
            mockPrisma.githubRepo.findMany.mockResolvedValueOnce([{ language: 'TS' }]).mockResolvedValueOnce([{ ownerName: 'me' }]);
    ...
    describe('getRecentActive', () => {
        it('按 repoUpdatedAt 降序获取最近活跃的 topN 仓库', async () => {
            const mockRepos = [{ id: 3, fullName: 'repo3', repoUpdatedAt: new Date('2025-01-01') }];
            mockPrisma.githubRepo.findMany.mockResolvedValue(mockRepos);
