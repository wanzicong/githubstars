...
export class PromptService {
     ...
     */
    async buildAgentTagPrompt(repoIndexText: string, repoCount: number, tagSystem: string): Promise<string> {
        const dimensionTable = await this.buildTagDimensionTable();
        const shorthands = this.getDimensionShorthands().join('/');

        return `你是 GitHub 项目分类专家。为以下 ${repoCount} 个项目打标签（每个项目 6-18 个标签）。
                ...
                4. **全维度覆盖**：每个项目必须在所有维度上都有标签（技术栈+领域+用途+服务人群+解决问题+状态），缺一不可。每个维度至少1个标签
                4a. **聚合优先**：标签应该遵循从聚合到发散的层级逻辑——
                    第一步：确定技术栈（语言/框架/运行时）
                    第二步：确定领域（AI/前端/后端/安全等）
                    第三步：确定用途（框架/工具/应用/库等）
                    第四步：确定服务人群（开发者/企业/个人等）
                    第五步：确定解决什么问题（自动化/效率/可视化等）
                    第六步：确定关注状态（活跃关注/学习参考/已归档等）
                4b. **高频复用**：优先选择 repoCount 高的已有标签，避免创建仅适用1-2个项目的零散标签。标签应该是聚合概念而非单个项目的描述词。
                5. **信息不足时**：描述/README 为空的项目，仅根据语言、Topics、项目名判断，标签宁少勿滥
                6. **标签精准**：同一维度下标签 1-3 个，宁精勿滥。标签命名应体现项目核心特征，避免用过于宽泛的词（如"工具"、"应用"）
                7. **新建标签**：中文命名，2-6字。无法归入现有维度时优先归入"用途"
                8. **只输出 JSON**，其他文字一律不要：
                \`\`\`
                {"0":["技术栈:Python","领域:AI/ML","用途:库/SDK"],"1":["技术栈:Rust","领域:系统工具","用途:CLI工具"]}
                \`\`\`
                9. **自查清单**（输出前逐项确认）：每个项目6个维度全覆盖？格式是"简称:名称"？维度简称在${shorthands}之中？无多余文字？`;
    }
}
