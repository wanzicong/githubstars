...
export class AuthorController {
    ...
    @ApiBody({
        schema: { type: 'object', properties: { page: { type: 'number' }, size: { type: 'number' }, keyword: { type: 'string' } } },
    })
    async list(@Body(new ZodValidationPipe(AuthorListSchema)) body: AuthorListDto) {
        return this.service.findAuthorPage(body.page, body.size, body.keyword);
    }
    ...
    @ApiBody({
        schema: {
            type: 'object',
            properties: { ownerName: { type: 'string' }, sortBy: { type: 'string' }, sortOrder: { type: 'string' } },
            required: ['ownerName'],
        },
    })
    async export(@Body(new ZodValidationPipe(AuthorExportSchema)) body: AuthorExportDto, @Res() res: Response) {
        const urls = await this.service.findAllAuthorRepoUrls({
            ownerName: body.ownerName,
            sortBy: body.sortBy,
            sortOrder: body.sortOrder,
        });
