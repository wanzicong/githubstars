...
export class TranslateController {
  ...
  async createTask(@Body() body: {
    ...
    filters?: {
      ...
      sortBy?: string; sortOrder?: string; dateField?: string
  ...
  async translationStatus(@Query() q: any) {
    return this.service.getTranslationSummary({
      ...
      categoryIds: q.categoryIds || '', dateField: q.dateField || '',
  ...
  async taskProgress(@Param('id') id: string) {
    const nid = this.parseId(id); if (!this.isValidId(nid)) return { success: false, message: '无效的任务ID' }
  ...
  async taskStream(@Param('id') id: string, @Res() res: Response) {
    ...
    res.on('close', () => { clearInterval(interval); sseStreams.delete(taskId) })
  }
  ...
  async translateDesc(@Param('id') id: string) {
    const nid = this.parseId(id); if (!this.isValidId(nid)) return { success: false, message: '无效的仓库ID' }
  ...
  async status(@Param('id') id: string) {
    ...
    const repo = await this.repoService.findById(nid)
    if (!repo) return { success: false, message: '仓库不存在' }
    return { success: true, descriptionTranslated: !!repo.descriptionCn, readmeFetched: repo.readmeFetched, readmeTranslated: !!repo.readmeCn, descriptionCn: repo.descriptionCn, readmeCn: repo.readmeCn }
  }
}
