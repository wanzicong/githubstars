...
export class CloneController {
  ...
  async config() { return this.cloneService.getCloneConfig() }
  ...
  async start(@Query() q: any) {
    return this.cloneService.startBatchClone({
      ...
      maxCount: parseInt(q.maxCount) || 50, subDirectory: q.subDirectory || '',
      dateField: q.dateField || '', startDate: q.startDate || '', endDate: q.endDate || '',
      sortBy: q.sortBy || 'starred_at', sortOrder: q.sortOrder || 'desc',
      concurrency: parseInt(q.concurrency) || 5, cloneDepth: parseInt(q.cloneDepth) || 1,
  ...
  async task(@Param('taskId') taskId: string) {
    ...
    return { success: true, taskId: t.taskId, status: t.status, totalRepos: t.totalRepos, completedRepos: t.completedRepos, failedRepos: t.failedRepos, skippedRepos: t.skippedRepos, cancelled: t.cancelled, results: (t as any).items?.map((i: any) => ({ fullName: i.fullName, status: i.status, message: i.message })) }
  }
  ...
  async script(@Query() q: any, @Res() res: Response) {
    const script = await this.cloneService.generateCloneScript({
      osType: q.osType || 'windows', keyword: q.keyword || '', language: q.language || '',
      categoryIds: q.categoryIds || '', maxCount: parseInt(q.maxCount) || 50,
      subDirectory: q.subDirectory || '', cloneDepth: parseInt(q.cloneDepth) || 1,
    })
    const ext = q.osType === 'linux' ? 'sh' : 'ps1'
    res.set({ 'Content-Type': 'text/plain; charset=utf-8', 'Content-Disposition': `attachment; filename*=UTF-8''${encodeURIComponent('clone-script.' + ext)}` })
    res.send(script)
  }
}
