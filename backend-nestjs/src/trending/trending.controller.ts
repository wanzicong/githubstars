...
export class TrendingController {
  constructor(private readonly search: GithubSearchService, private readonly ai: AiAnalyzeService) {}
  ...
  async trending(@Query() q: any) {
    ...
    return { success: true, since, total: result.total, repos: result.repos, dateRange: `${dateStr} ~ ${new Date().toISOString().split('T')[0]}` }
  }
