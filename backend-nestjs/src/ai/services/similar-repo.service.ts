...
export class SimilarRepoService {
  ...
  private async callDeepSeek(prompt: string, systemMsg?: string): Promise<string | null> {
    ...
    try {
      const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify({ model, temperature: 0.5, max_tokens: 2048, messages: msgs }) })
  ...
  private async searchAndConvert(query: string): Promise<any[]> {
    ...
    return items.map((item: any) => ({
      fullName: item.full_name || '', description: item.description || '',
      language: item.language || '', stars: item.stargazers_count || 0,
      forks: item.forks_count || 0, htmlUrl: item.html_url || '',
      pushedAt: item.pushed_at || '', aiReason: '',
  ...
  private async enrichWithAI(repos: any[], source: any) {
    ...
    try {
      const result = await this.callDeepSeek(prompt, '你是GitHub项目推荐专家。只返回要求格式的JSON，不说任何废话。')
      if (result) {
        const reasons = JSON.parse(result.replace(/^```json\s*/i, '').replace(/```$/i, '').trim()) as string[]
        if (Array.isArray(reasons)) top.forEach((r, i) => { r.aiReason = reasons[i] || '' })
      }
    } catch { this.logger.warn('AI推荐理由生成失败') }
  }

  async findSimilar(repoId: number): Promise<any[]> {
    const source = await this.githubRepo.findById(repoId)
    if (!source) return []
    const seen = new Set<string>(); seen.add(source.fullName || '')
    ...
    for (const kw of keywords) {
      ...
      for (const r of await this.searchAndConvert(q)) {
        if (!seen.has(r.fullName)) { seen.add(r.fullName); allResults.push(r) }
      }
    }
