...
export class AiClassifyService {
  ...
  private async callDeepSeek(prompt: string, systemMsg?: string): Promise<string | null> {
    ...
    try {
      const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify({ model, temperature: 0.3, max_tokens: 4096, messages: msgs }) })
      if (!res.ok) return null
      const data = await res.json() as any
      let content = data.choices?.[0]?.message?.content?.trim() || null
      if (content) content = content.replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/```$/i, '').trim()
      return content
    } catch { return null }
  }
...
只返回 JSON：\n{"分类名1": [序号], "分类名2": [序号]}\n\n${list}`
  ...
  async smartClassify(repoIds: number[]) {
    ...
    for (const name of Object.keys(assignments)) { if (allNames.includes(name)) matchCount++; else newCount++ }
