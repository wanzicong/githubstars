...
export class CategoryController {
  constructor(private readonly service: CategoryService, private readonly aiClassify: AiClassifyService) {}

  @Get('categories/all')
  async all() { return this.service.listAll() }
  ...
  async delete(@Param('id') id: string) {
    try { await this.service.delete(parseInt(id)); return { success: true, message: '删除成功' } }
    catch (e) { return { success: false, message: e instanceof Error ? e.message : String(e) } }
  }
  ...
  async move(@Param('id') id: string, @Body() b: any) {
    try { await this.service.moveToParent(parseInt(id), b.parentId); return { success: true, message: '移动成功' } }
    catch (e) { return { success: false, message: e instanceof Error ? e.message : String(e) } }
  }
  ...
  async repos(@Param('id') id: string, @Query() q: any) {
    ...
    if (page) {
      return this.service.getReposByCategoryIdPaged({
        categoryId: parseInt(id), page: parseInt(page), size: parseInt(q.size) || 12,
  ...
  async addRepos(@Param('id') id: string, @Body() b: any) {
    try { await this.service.batchAddRepos(b.repoIds, parseInt(id)); return { success: true, message: '添加成功' } }
    catch (e) { return { success: false, message: e instanceof Error ? e.message : String(e) } }
  }

  @Delete('categories/:categoryId/repos/:repoId')
  async removeRepo(@Param('categoryId') catId: string, @Param('repoId') repoId: string) {
    try { await this.service.removeRepoFromCategory(parseInt(repoId), parseInt(catId)); return { success: true, message: '移除成功' } }
    catch (e) { return { success: false, message: e instanceof Error ? e.message : String(e) } }
  }

  @Post('categories/:id/repos/transfer')
  async transfer(@Param('id') id: string, @Body() b: any) {
    try { await this.service.batchTransferRepos(b.repoIds, parseInt(id), b.toCategoryId); return { success: true, message: '转移成功' } }
    catch (e) { return { success: false, message: e instanceof Error ? e.message : String(e) } }
  }

  @Get('categories/uncategorized')
  async uncategorized() { return this.service.getUncategorized() }
  ...
  async reclassify(@Param('id') id: string, @Body() b: any) {
    try {
      ...
      const result = await this.aiClassify.classify(repos.map(r => Number(r.id)), b?.topN || 8)
      return result
    } catch (e) { return { success: false, message: '重分类失败: ' + (e instanceof Error ? e.message : String(e)) } }
  }
  ...
  async smartClassify() {
    ...
    } catch (e) { return { success: false, message: '智能分类失败: ' + (e instanceof Error ? e.message : String(e)) } }
  }
}
