...
describe('CategoryService', () => {
  ...
  describe('listAll — 树形结构', () => {
    it('扁平分类应构建正确的树形嵌套', async () => {
      const categories = [
        { id: BigInt(1), name: 'AI', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
        { id: BigInt(2), name: 'LLM', level: 2, parentId: BigInt(1), sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
        { id: BigInt(3), name: 'Agent', level: 2, parentId: BigInt(1), sortOrder: 1, description: null, createdAt: new Date(), updatedAt: null },
        { id: BigInt(4), name: 'Frontend', level: 1, parentId: null, sortOrder: 1, description: null, createdAt: new Date(), updatedAt: null },
      ]
    ...
    it('level=2 分类无有效 parentId 时应作为根节点', async () => {
      ...
      const categories = [
        { id: BigInt(1), name: 'Orphan', level: 2, parentId: BigInt(99), sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
        { id: BigInt(2), name: 'Root', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
      ]
    ...
    it('根节点应按 repoCount 降序排列', async () => {
      const categories = [
        { id: BigInt(1), name: 'Small', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
        { id: BigInt(2), name: 'Large', level: 1, parentId: null, sortOrder: 1, description: null, createdAt: new Date(), updatedAt: null },
      ]
    ...
    it('子分类 repoCount 为 0 时不影响父级汇总', async () => {
      const categories = [
        { id: BigInt(1), name: 'Empty', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
        { id: BigInt(2), name: 'Sub', level: 2, parentId: BigInt(1), sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null },
      ]
  ...
  describe('create — 唯一性校验', () => {
    it('名称重复时应抛出异常', async () => {
      mockPrisma.category.findUnique.mockResolvedValue({
        id: BigInt(1), name: 'Duplicated', level: 1, parentId: null,
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
    ...
    it('名称不重复时应成功创建', async () => {
      ...
      const created = { id: BigInt(99), name: 'NewCat', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null }
  ...
  describe('delete — 级联删除', () => {
    it('应先删除 repo_category 关联再删除分类', async () => {
      ...
      // 验证调用顺序
      const calls = [
        mockPrisma.repoCategory.deleteMany,
  ...
  describe('batchAddRepos', () => {
    it('skipDuplicates=true: createMany 使用 skipDuplicates 选项', async () => {
      mockPrisma.category.findUnique.mockResolvedValue({
        id: BigInt(2), name: 'Sub', level: 2, parentId: BigInt(1),
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
    ...
    it('level=1 分类不能直接包含仓库，应抛出异常', async () => {
      mockPrisma.category.findUnique.mockResolvedValue({
        id: BigInt(1), name: 'Root', level: 1, parentId: null,
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
  ...
  describe('saveAiClassifyResult', () => {
    it('应为每个分类清除现有关联后添加新关联', async () => {
      ...
      mockPrisma.category.findUnique.mockResolvedValueOnce({
        id: BigInt(1), name: 'AI', level: 1, parentId: null,
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
      ...
      mockPrisma.category.create.mockResolvedValue({
        id: BigInt(2), name: 'Frontend', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
    ...
    it('空 ids 数组的分类应跳过', async () => {
      ...
      mockPrisma.category.create.mockResolvedValue({
        id: BigInt(2), name: 'RealCat', level: 1, parentId: null, sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
  ...
  describe('getUncategorized', () => {
    it('应使用原始 SQL 查询无分类的仓库', async () => {
      ...
      expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(
        expect.stringContaining('LEFT JOIN repo_category'),
      )
    })
  ...
  describe('expandCategoryIds', () => {
    ...
    it('level=2 分类应原样保留 ID', async () => {
      mockPrisma.category.findUnique.mockResolvedValueOnce({
        id: BigInt(2), name: 'LLM', level: 2, parentId: BigInt(1),
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
    ...
    it('level=1 分类无子分类时应返回自身 ID', async () => {
      mockPrisma.category.findUnique.mockResolvedValueOnce({
        id: BigInt(1), name: 'EmptyParent', level: 1, parentId: null,
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
      mockPrisma.category.findMany.mockResolvedValueOnce([])
    ...
    it('多个混合 ID 应同时处理', async () => {
      ...
      // id=4: level=2 → 保留
      mockPrisma.category.findUnique.mockResolvedValueOnce({
        id: BigInt(4), name: 'React', level: 2, parentId: BigInt(5),
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      })
  ...
  describe('getById', () => {
    it('应查询指定 ID 的分类', async () => {
      const cat = {
        id: BigInt(1), name: 'Test', level: 1, parentId: null,
        sortOrder: 0, description: null, createdAt: new Date(), updatedAt: null,
      }
