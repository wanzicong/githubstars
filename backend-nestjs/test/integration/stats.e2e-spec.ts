...
describe('stats (e2e)', () => {
  ...
  describe('GET /api/stats/owners', () => {
    ...
    it('should default topN to 15 when given invalid value', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/stats/owners?topN=invalid')
        .expect(200)
  ...
  describe('GET /api/stats/top-starred', () => {
    ...
    it('should respect custom topN parameter', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/stats/top-starred?topN=3')
        .expect(200)
    ...
    it('should default topN to 10 when given invalid value', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/stats/top-starred?topN=invalid')
        .expect(200)
  ...
  describe('GET /api/stats/recent-active', () => {
    it('should return repos sorted by repoUpdatedAt descending', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/stats/recent-active')
        .expect(200)

      expect(res.body).toBeInstanceOf(Array)
      expect(res.body.length).toBe(3)
      // 按 repoUpdatedAt 降序
      expect(new Date(res.body[0].repoUpdatedAt).getTime()).toBeGreaterThanOrEqual(
        new Date(res.body[1].repoUpdatedAt).getTime(),
      )
      expect(new Date(res.body[1].repoUpdatedAt).getTime()).toBeGreaterThanOrEqual(
        new Date(res.body[2].repoUpdatedAt).getTime(),
      )
    })
    ...
    it('should respect custom topN parameter', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/stats/recent-active?topN=3')
        .expect(200)
