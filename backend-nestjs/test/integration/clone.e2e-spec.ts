...
async function insertCloneTaskItem(
  prisma: PrismaService,
  data: { taskId: string; fullName: string; status: string; message?: string },
) {
...
describe('Clone API (e2e)', () => {
  ...
  describe('GET /api/clone/disk-space', () => {
    ...
    it('传入 repoCount 参数估算值相应变化', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/clone/disk-space?repoCount=10')
        .expect(200)
    ...
    it('传入 subDirectory 仍正常返回（可能创建目录失败但不阻断响应）', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/clone/disk-space?subDirectory=e2e_test_check&repoCount=5')
        .expect(200)
  ...
  describe('GET /api/clone/tasks', () => {
    ...
    it('分页参数 page/size 被正确传递', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/clone/tasks?page=1&size=2')
        .expect(200)
    ...
    it('第二页数据正确（含 page=2）', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/clone/tasks?page=2&size=2')
        .expect(200)
    ...
    it('超出范围的 page 返回空 records 但 total 不变', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get('/api/clone/tasks?page=99&size=20')
        .expect(200)
  ...
  describe('GET /api/clone/tasks/:taskId', () => {
    it('任务不存在 → 返回失败', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/clone/tasks/nonexistent-task-id')
        .expect(200)
    ...
    it('status 过滤仅返回匹配状态的子项', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get(`/api/clone/tasks/${t.taskId}?status=FAILED`)
        .expect(200)
  ...
  describe('DELETE /api/clone/tasks/:taskId', () => {
    it('删除任务后任务列表不再包含该任务', async () => {
      ...
      const del = await request(app.getHttpServer())
        .delete(`/api/clone/tasks/${t.taskId}`)
        .expect(200)
  ...
  describe('POST /api/clone/tasks/:taskId/pin', () => {
    it('置顶未置顶任务 → 返回 pinned=true', async () => {
      ...
      const res = await request(app.getHttpServer())
        .post(`/api/clone/tasks/${t.taskId}/pin`)
        .expect(201)
    ...
    it('不存在的任务 → 返回失败', async () => {
      const res = await request(app.getHttpServer())
        .post('/api/clone/tasks/fake_task_999/pin')
        .expect(201)
  ...
  describe('POST /api/clone/tasks/:taskId/cancel', () => {
    it('取消 PENDING 任务 → 成功', async () => {
      ...
      const res = await request(app.getHttpServer())
        .post(`/api/clone/tasks/${t.taskId}/cancel`)
        .expect(201)
    ...
    it('取消不存在的任务 → 失败', async () => {
      const res = await request(app.getHttpServer())
        .post('/api/clone/tasks/no_such_task/cancel')
        .expect(201)
  ...
  describe('GET /api/clone/tasks/:taskId/items', () => {
    it('任务有子项 → 返回完整子项列表含分页', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get(`/api/clone/tasks/${t.taskId}/items`)
        .expect(200)
    ...
    it('status 过滤 SKIPPED → 仅返回 SKIPPED 项', async () => {
      ...
      const res = await request(app.getHttpServer())
        .get(`/api/clone/tasks/${t.taskId}/items?status=SKIPPED`)
        .expect(200)
