...
CREATE TABLE IF NOT EXISTS `clone_task` (
    ...
    `clone_depth` INT DEFAULT 1 COMMENT '克隆深度：1=浅克隆，0=完整克隆',
    `max_repo_size_mb` INT DEFAULT 500 COMMENT '跳过超过此大小(MB)的仓库，0=不限制',
    `started_at` DATETIME DEFAULT NULL COMMENT '开始时间',
    `finished_at` DATETIME DEFAULT NULL COMMENT '完成时间',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `pinned` INT DEFAULT 0 COMMENT '是否置顶：0=否，1=是',
    `cancelled` INT DEFAULT 0 COMMENT '是否已取消：0=否，1=是',
    PRIMARY KEY (`id`),
