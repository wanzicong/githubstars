...
import { Modal, Button, Progress, Space, Statistic, Row, Col, Card, Tag, Table, Typography, Divider, Alert, message, Empty, Descriptions, Badge, Tooltip } from 'antd'
import {
  TranslationOutlined, ReloadOutlined, BarChartOutlined,
  CheckCircleOutlined, CloseCircleOutlined, ClockCircleOutlined,
  ReadOutlined, FileTextOutlined, FilterOutlined, SyncOutlined,
  ApartmentOutlined,
...
export default function TranslatePanel({ open, onClose, filters, hasActiveFilters, onRefreshList }: Props) {
  ...
  const [coverage, setCoverage] = useState<{
    descCompleted: number; descPending: number; readmeCompleted: number; readmePending: number; total: number
  } | null>(null)

  // 活跃任务
  const [activeTaskId, setActiveTaskId] = useState<number | null>(null)
  const [taskProgress, setTaskProgress] = useState<{
    status: string; totalItems: number; completedItems: number; failedItems: number; pendingItems: number
    descTotal: number; descCompleted: number; descFailed: number
    readmeTotal: number; readmeCompleted: number; readmeFailed: number
    progress: number
  } | null>(null)
  ...
  const handleStartTranslate = async (type: 'description' | 'readme' | 'both') => {
    ...
    try {
      ...
      if (res.success && res.taskId) {
        ...
        setTaskProgress({ status: 'PENDING', totalItems: 0, completedItems: 0, failedItems: 0, pendingItems: 0, descTotal: 0, descCompleted: 0, descFailed: 0, readmeTotal: 0, readmeCompleted: 0, readmeFailed: 0, progress: 0 })
        startPolling(res.taskId)
        message.success('翻译任务已启动')
      } else {
        message.info(res.message || '没有需要翻译的项目')
      }
    } catch { message.error('启动翻译失败') }
  ...
  return (
    <Modal
      title={<Space><TranslationOutlined /> 翻译管理</Space>}
      open={open}
      onCancel={onClose}
      width={760}
        ...
        <Row gutter={24}>
          <Col span={12}>
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              <Progress percent={descPercent} size="small"
          ...
          <Col span={12}>
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              <Progress percent={readmePercent} size="small"
        ...
        <Space wrap>
          ...
          <Tooltip title="翻译描述 + README（全量翻译）">
            <Button
              ...
              onClick={() => handleStartTranslate('both')}
            >
              翻译全部 (描述+README)
            </Button>
          </Tooltip>
        ...
        <Card size="small" style={{ marginBottom: 16, borderColor: isRunning ? '#1677ff' : undefined }}>
          <Space style={{ width: '100%', justifyContent: 'space-between' }}>
            <Text strong>
              ...
              {' '}任务 #{String(activeTaskId).slice(-6)}
              {isRunning ? <Tag color="processing" style={{ marginLeft: 8 }}>进行中</Tag>
                : taskProgress.status === 'COMPLETED' ? <Tag color="success">完成</Tag>
                  : taskProgress.status === 'PARTIAL' ? <Tag color="warning">部分失败</Tag>
          ...
          <Row gutter={16}>
            ...
            <Col span={8}><Statistic title="失败" value={taskProgress.failedItems} valueStyle={{ color: taskProgress.failedItems > 0 ? '#cf1322' : undefined }} /></Col>
          </Row>
        ...
        <Card size="small" title={<Space><ClockCircleOutlined /> 最近任务</Space>}>
          <Table
            ...
            columns={[
              ...
              { title: '失败', dataIndex: 'failedItems', width: 60, render: (v: number) => v > 0 ? <Text type="danger">{v}</Text> : <Text type="secondary">0</Text> },
              { title: '时间', dataIndex: 'createdAt', render: (v: string) => v ? new Date(v).toLocaleString('zh-CN') : '-' },
            ]}
          />
        </Card>
      )}
