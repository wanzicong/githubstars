    ...
    const itemColumns = [
        {
            ...
            key: 'fullName',
            render: (v: string) => (
                <a href={`https://github.com/${v}`} target='_blank' rel='noopener noreferrer' onClick={(e) => e.stopPropagation()}>
                    {v}
                </a>
            ),
        },
    ...
    if (!task) {
        return (
            <div style={{ textAlign: 'center', padding: 80 }}>
                ...
                <Button type='primary' onClick={() => navigate('/clone-tasks')}>
                    返回任务列表
                </Button>
            </div>
        )
                ...
                <Card style={{ marginBottom: 16 }}>
                    <Descriptions title='基本信息' column={{ xs: 1, sm: 2, md: 3 }} size='small'>
                        ...
                        <Descriptions.Item label='状态'>
                            <Tag color={STATUS_COLOR_MAP[task.status] || 'default'}>{STATUS_LABEL_MAP[task.status] || task.status}</Tag>
                        </Descriptions.Item>
                            ...
                            <Progress
                                percent={
                                    progress.totalRepos > 0
                                        ? Math.round(
                                              ((progress.completedRepos + progress.failedRepos + progress.skippedRepos) /
                                                  progress.totalRepos) *
                                                  100,
                                          )
                                        : 0
