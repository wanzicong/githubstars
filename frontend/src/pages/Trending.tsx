...
export default function Trending() {
  ...
  useEffect(() => { load(since, language) }, [since, language, load])
  ...
  return (
    <div>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        ...
        <Space>
          <Select value={language || ''} onChange={(v) => setLanguage(v)}
            options={LANGUAGE_OPTIONS} style={{ width: 140 }} placeholder="语言" />
          <Segmented value={since} onChange={(v) => { setSince(v as string) }}
            options={[
      ...
      <Spin spinning={loading}>
        {dateRange && <Text type="secondary" style={{ display: 'block', marginBottom: 16, fontSize: 12 }}>统计时段: {dateRange} | 共 {total} 个新仓库</Text>}
                ...
                <Card hoverable size="small" styles={{ body: { padding: 16 } }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{
                      ...
                      fontWeight: 700, fontSize: idx < 3 ? 16 : 13
                    }}>
                      {idx + 1}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        <img src={repo.owner_avatar_url} alt="" style={{ width: 18, height: 18, borderRadius: '50%' }} />
      ...
      <Modal title={<Space><BulbOutlined style={{ color: '#faad14' }} />AI 趋势分析报告</Space>}
        open={analyzeModalVisible} onCancel={() => { setAnalyzeModalVisible(false); if (analyzePollRef.current) clearInterval(analyzePollRef.current) }}
        footer={<Button type="primary" onClick={() => setAnalyzeModalVisible(false)}>关闭</Button>}
        width={900} style={{ top: 20 }} maskClosable={analyzeStatus === 'COMPLETED'}>
        {analyzeStatus === 'PROCESSING' && (
          <div style={{ textAlign: 'center', padding: 40 }}>
            <Spin size="large" /><div style={{ marginTop: 16 }}>AI 正在分析趋势数据...</div>
          </div>
        )}
