...
export default function StarList() {
    ...
    const [cloneProgress, setCloneProgress] = useState<{
        ...
        failedRepos: number
        skippedRepos: number
        results: { fullName: string; status: string; message: string }[]
    } | null>(null)
    ...
    const cloneSubDirOptions = useMemo(() => cloneSubDirHistory.map((dir) => ({ value: dir, label: dir })), [cloneSubDirHistory])
                    ...
                    <Row gutter={[8, 8]} style={{ marginTop: 8 }}>
                        <Col span={24}>
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                                ...
                                <Button icon={<DownloadOutlined />} onClick={handleOpenCloneDirModal} disabled={cloneInProgress}>
                                    批量Clone
                                </Button>
            ...
            >
                {analyzeStatus === 'PROCESSING' && (
                    <div style={{ textAlign: 'center', padding: 40 }}>
                        ...
                        <div style={{ marginTop: 8, fontSize: 13, color: '#999' }}>最多分析 30 个项目 | 分析内容包括描述和 README</div>
                    </div>
                )}
            ...
            >
                <Spin spinning={cloneDirLoading}>
                    ...
                    <Alert
                        ...
                        description={
                            cloneSubDirHistory.length > 0
                                ? '可从下拉列表选择历史子目录，也可输入新目录'
                                : '首次使用可直接输入子目录，任务成功完成后会自动保存到历史记录'
                        }
                    />
                </Spin>
            </Modal>
            ...
            <Modal
                ...
                footer={
                    <Space>
                        ...
                        <Button type='primary' onClick={handleCloseCloneModal}>
                            关闭
                        </Button>
                    </Space>
                }
                    ...
                    <div>
                        ...
                        {cloneProgress.status === 'FAILED' && (
                            <Alert
                                ...
                                showIcon
                                message='Clone 任务失败'
                                description={cloneProgress.errorMessage || '未知错误'}
                                style={{ marginBottom: 12 }}
                            />
                        )}
                        <div style={{ marginBottom: 16, display: 'flex', gap: 16, justifyContent: 'center' }}>
                            <span>
                                总数: <Text strong>{cloneProgress.totalRepos}</Text>
                            </span>
                            <span style={{ color: '#52c41a' }}>
                                成功: <Text strong>{cloneProgress.completedRepos}</Text>
                            </span>
                            <span style={{ color: '#faad14' }}>
                                跳过: <Text strong>{cloneProgress.skippedRepos}</Text>
                            </span>
                            <span style={{ color: '#ff4d4f' }}>
                                失败: <Text strong>{cloneProgress.failedRepos}</Text>
                            </span>
                        </div>
                        {cloneProgress.status === 'RUNNING' && (
                            <Spin tip='正在 Clone...' style={{ display: 'block', textAlign: 'center' }} />
                        )}
                                ...
                                {cloneProgress.results.map((r, i) => (
                                    ...
                                    >
                                        <span
                                            style={{
                                                minWidth: 60,
                                                color: r.status === 'CLONED' ? '#52c41a' : r.status === 'SKIPPED' ? '#faad14' : '#ff4d4f',
                                            }}
                                        >
