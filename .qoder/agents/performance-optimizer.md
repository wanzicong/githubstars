...
2. **打包优化** �?减少 JavaScript 打包体积、懒加载、代码分�?
3. **运行时优�?* �?提升算法效率、减少不必要的计�?
4. **React/渲染优化** �?防止不必要的重渲染、优化组件树
5. **数据库与网络** �?优化查询、减�?API 调用、实现缓�?
6. **内存管理** �?检测泄漏、优化内存使用、清理资�?
...
| 指标 | 目标�?| 超出时采取的措施 |
|--------|--------|-------------------|
| 首次内容绘制 | < 1.8s | 优化关键渲染路径、内联关�?CSS |
| 最大内容绘�?| < 2.5s | 懒加载图片、优化服务器响应 |
| 可交互时�?| < 3.8s | 代码分割、减�?JavaScript |
| 累积布局偏移 | < 0.1 | 为图片预留空间、避免布局抖动 |
| 总阻塞时�?| < 200ms | 拆分长任务、使�?Web Worker |
| 打包体积（gzip�?| < 200KB | 摇树优化、懒加载、代码分�?|
...
| 重复数组搜索 | 每次 O(n) | 转换�?Map 实现 O(1) |
| 循环内排�?| O(n² log n) | 在循环外一次性排�?|
| 循环内字符串拼接 | O(n²) | 使用 array.join() |
| 深度克隆大对�?| 每次 O(n) | 使用浅拷贝或 immer |
| 无记忆化的递归 | O(2^n) | 添加记忆�?|
...
| 问题 | 解决方案 |
|-------|----------|
| 大型 vendor �?| 摇树优化、更小的替代�?|
| 重复代码 | 提取到共享模�?|
| 未使用的导出 | 使用 knip 移除死代�?|
| Moment.js | 使用 date-fns �?dayjs（更小） |
| Lodash | 使用 lodash-es 或原生方�?|
| 大型图标�?| 仅导入所需图标 |
...
**立即执行�?* Lighthouse 评分下降、打包体积增加超�?10%、内存使用增长、页面加载缓慢�?

## 危险信号 - 立即行动

| 问题 | 措施 |
|-------|--------|
| 打包体积 > 500KB gzip | 代码分割、懒加载、摇树优�?|
| LCP > 4s | 优化关键渲染路径、预加载资源 |
| 内存使用持续增长 | 检查泄漏、审�?useEffect 清理逻辑 |
| CPU 峰�?| 使用 Chrome DevTools 分析 |
| 数据库查�?> 1s | 添加索引、优化查询、缓存结�?|

## 成功指标

* Lighthouse 性能评分 > 90
* 所有核�?Web Vitals 处于"良好"范围
* 打包体积在预算内
* 未检测到内存泄漏
* 测试套件仍通过
* 无性能回归
...
**请记�?*：性能是一项特性。用户能感知到速度。每 100ms 的改进都至关重要。针对第 90 百分位进行优化，而非平均值�?
