...
description: 分叉任何项目以进行开源。复制文件，剥离机密和凭据（20多种模式），用占位符替换内部引用，生�?env.example，并清理git历史。这是opensource-pipeline技能的第一阶段�?
...
* 从源文件中剥离所有机密信息、凭据和令牌
* 将内部引用（域名、路径、IP）替换为可配置的占位�?
* 从每个提取的值生�?`.env.example`
* 创建全新�?Git 历史（单个初始提交）
* 生成 `FORK_REPORT.md` 记录所有变�?
...
| 绝对主目录路�?`/home/username/` | `/home/user/` �?`$HOME/` |
| 机密文件引用 `~/.secrets/` | `.env` |
| 私有 IP `192.168.x.x`、`10.x.x.x` | `your-server-ip` |
| 内部服务 URL | 通用占位�?|
| 个人邮箱地址 | `you@your-domain.com` |
| 内部 GitHub 组织�?| `your-github-org` |

保留功能完整性——每次替换都需�?`.env.example` 中有对应条目�?
...
操作：复制文件，�?`DATABASE_URL` 中剥�?`docker-compose.yml`，将 `internal.company.com` 替换�?`your-domain.com`，创建包�?8 个变量的 `.env.example`，全�?git init
输出：`FORK_REPORT.md` 列出所有变更，临时目录已准备好供清理工具处�?

## 规则

* **绝不**在输出中遗留任何机密信息，即使被注释掉也不行
* **绝不**移除功能——始终参数化，不要删除配�?
* **始终**为每个提取的值生�?`.env.example`
